...
exportWig <- function(bam_file = '', bin_width = 50L, shift_reads = 0L,outdir='./',col=c(200,0,0)) {
  ...
  covobj <- do.call('c', mclapply(chr_list, function(tmp_chr) {
    ...
    if (shift_reads > 0L) {
      ...
      read_ranges <- IRanges(
        start = ifelse(
          reads[['strand']][reads[['rname']] == tmp_chr] == '+',
          reads[['pos']][reads[['rname']] == tmp_chr] + shift_reads,
          reads[['pos']][reads[['rname']] == tmp_chr] - shift_reads
        ),
        strand=reads[['strand']][reads[['rname']] == tmp_chr],#add in strand to the iranegs object
        width = rwidth
      )
    } else {
      read_ranges <- IRanges(start = reads[['pos']][reads[['rname']] == tmp_chr], width = rwidth)#add in strand to the iranegs object
                            
    }
    
    read_score <- Rle(countOverlaps(bin_ranges[[tmp_chr]], read_ranges, type = 'any'))
