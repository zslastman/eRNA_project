...
nallmap=!allmap
cg=mclapply(mc.cores=5,names(cg),function(acc){
...
actual values for 1s in each library a reasonable multiple
...
################################################################################
#7b loand for the normalized data
cg.merge.pl=cg.pl

message('CAGE Rle objects calculated, now summing reseqs')
reseqs<-names(cg.merge.pl)[grepl(names(cg.merge.pl),pattern='reseq')]

for(reseq in reseqs){
  orig<-gsub(reseq,pattern='_reseq',replacement='')
  cg.merge.pl[[orig]]$pos<-cg.merge.pl[[orig]]$pos+cg.merge.pl[[reseq]]$pos
  cg.merge.pl[[orig]]$neg<-cg.merge.pl[[orig]]$pos+cg.merge.pl[[reseq]]$neg
}

#now get rid of the reseqs  
cg.merge.pl<-cg.merge.pl[!names(cg.merge.pl) %in% reseqs]

message('CAGE Rle objects calculated, now summing replicates')

replicates<-names(cg.merge.pl)[grepl(names(cg.merge.pl),pattern='(+\\d)_?r2')]  
for(replicate in replicates){
  orig<-gsub(reseq,pattern='(+\\d)_?r2',replacement='\\1')
  cg.merge.pl[[orig]]$pos<-cg.merge.pl[[orig]]$pos+cg.merge.pl[[replicate]]$pos
  cg.merge.pl[[orig]]$neg<-cg.merge.pl[[orig]]$pos+cg.merge.pl[[replicate]]$neg
}

#now get rid of the replicates  
cg.merge.pl<-cg.merge.pl[!names(cg.merge.pl) %in% replicates]
save(cg.merge.pl,file='data/objects/cg.pl.merge.object.R')
